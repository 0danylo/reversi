...
class TriangleReversiBot:
    def __init__(self, depth=6, max_time=2.75, max_tt_size=100000, tt_file="triangle_tt.pkl"):
        ...
        # Evaluation Weights (Fine-tuned)
        self.W_CORNER = 500
        self.W_TOP_CORNER = 100
        self.W_EDGE = 50 # Increased from 30
        self.W_BAD = -50
        self.W_NORMAL = 10
        self.W_MOBILITY = 10
        ...
        self.MASK_EDGE = 0
        self.MASK_NORMAL = 0
        # Note: MASK_BAD is removed in favor of dynamic check, 
        # but we keep a mask for static fallback if needed or for visualization
        ...
        for r in range(self.ROWS):
            for c in range(self.OFFSETS[r], self.OFFSETS[r] + self.LENGTHS[r]):
                ...
                # (8,1) etc are now handled dynamically, so treat them as normal/edge for static mask
