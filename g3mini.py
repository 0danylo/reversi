...
class ReversiBot:
    def __init__(self, depth=100, max_time=2.95, max_tt_size=50000, tt_file="tt.pkl"):
        ...
        if os.path.exists(self.tt_file):
            try:
                self.tt = pickle.load(open(self.tt_file, "rb"))
            except Exception:
                self.tt = OrderedDict()
        else:
            self.tt = OrderedDict()
    ...
    def save_tt(self):
        temp_file = f"{self.tt_file}.{os.getpid()}.tmp"
        try:
            pickle.dump(self.tt, open(temp_file, "wb"), protocol=pickle.HIGHEST_PROTOCOL)
            os.replace(temp_file, self.tt_file)
        except Exception:
    ...
    def choose_move(self, board, me=1, opp=2):
        ...
        save_buffer = max(0.05, self.load_duration * 1.25)
...
def main():
    ...
    bot = ReversiBot(depth=84, max_time=2.9)
